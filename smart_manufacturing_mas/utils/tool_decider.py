"""
ToolDecider - Rule-based interface for agent-level tool selection decisions.

This module provides a deterministic, rule-based interface for making tactical decisions
about preprocessing tools, model selection, and hyperparameters.

Architecture Note (SLM Reduction 4 -> 1):
- This module is part of TIER 3 (Rule-Based) in the Three-Tier Intelligence Hierarchy
- Replaces SLM positions 1, 2, and 3a with deterministic if-else logic
- Zero latency, zero hallucination risk
- Covers all decisions where the input space is fully enumerable

The LLMToolDecider class is kept for backward compatibility but the system
now defaults to RuleBasedToolDecider for all preprocessing and model selection.
"""
