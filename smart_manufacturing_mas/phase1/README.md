...
The runner prefers Parquet through `pyarrow`. If no Parquet engine is available, it falls back to CSV and updates catalogue/manifest paths to the files actually written. The three artifact directories use safe symlinks when possible, so identical representations do not consume three times the disk space.
...
The three artifact directories use safe symlinks when possible, so identical representations do not consume three times the disk space and can be regenerated independently.
