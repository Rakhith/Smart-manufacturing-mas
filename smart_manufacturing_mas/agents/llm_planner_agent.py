...
class LLMPlannerAgent:
    """
    LLM Planner Agent for orchestrating the MAS workflow.
    
    Architecture (Three-Tier Intelligence Hierarchy):
    - TIER 1: Cloud LLM (Gemini 2.5-Flash) - Strategic Orchestration
      * Workflow planning and chain-of-thought reasoning
      * Error recovery and adaptive retry
      * Reflexion loop for narrative summary generation
    
    - TIER 2: Local SLM (Qwen3:4B via Ollama) - Tactical Edge Inference
      * ONLY used for anomaly detection parameters (contamination, n_estimators)
      * All other decisions moved to rule-based or Cloud LLM
    
    - TIER 3: Rule-Based ToolDecider - Deterministic Decisions
      * Imputer choice, scaler choice, model family selection
      * Zero latency, zero hallucination risk
    
    SLM Reduction (4 -> 1):
    - SLM 1 (Perception): ELIMINATED - Uses pandas dtypes + HITL operator selection
    - SLM 2 (Preprocessing): ELIMINATED - Uses rule-based ToolDecider
    - SLM 3a (Model Selection): ELIMINATED - Uses ToolDecider rule table
    - SLM 3b (Anomaly Params): RETAINED - Only SLM call remaining
    - SLM 4 (Summary): ELIMINATED - Uses Cloud LLM Reflexion loop
    """
    @staticmethod
    ...
    def __init__(self, dataset_path: str, feature_columns: list, target_column: str, problem_type: str,
        ...
        Architecture (SLM Reduction 4 -> 1):
        - llm_agent: Planner LLM (Gemini) for workflow orchestration, HITL, AND Reflexion summary
        - decision_llm_agent: Tactical SLM (Qwen/Ollama) - ONLY used for anomaly detection params
        - All other decisions (preprocessing, model selection) use rule-based ToolDecider
        
        Args:
            llm_agent: Planner LLM (Gemini or similar) for workflow orchestration and HITL.
                       If None, defaults to Gemini 2.5-Flash.
            decision_llm_agent: Tactical SLM (Qwen/Ollama) for anomaly detection parameters ONLY.
                               If None, uses template fallback for anomaly params.
                               NOTE: This is the ONLY remaining SLM call in the system.
            hitl_interface: HITL interface for user interactions (defaults to CLI)
        """
        logging.info("Initializing LLM Planner Agent...")
        logging.info("Architecture: Cloud LLM (orchestration + Reflexion summary) | Local SLM (anomaly params only) | Rule-based (preprocessing + model selection)")
        load_dotenv()
        self.llm_agent = llm_agent
        self.decision_llm_agent = decision_llm_agent
        self.hitl_interface = hitl_interface or get_hitl_interface("cli")
        
        # Initialize Cloud LLM (Gemini) for orchestration AND Reflexion summary
        if self.llm_agent is None:
            ...
            self.model = None  # Using provided llm_agent instead
        ...
        # Create summarizer with Cloud LLM for Reflexion loop (SLM 4 eliminated)
        # Pass the Gemini model for Reflexion, NOT the local SLM
        self.summarizer = create_summarizer(cloud_llm_model=self.model, llm_agent=None)
        self.summarizer.set_logging_mode("minimal")  # Keep all existing logs, add minimal summarizer logs
    ...
    def run_workflow_with_llm(self, goal: str):
        ...
        for step in range(max_steps):
            ...
            # PATCH 1: Check finish=True BEFORE tool-name validation.
            # When the LLM signals it is done (finish=True) it may return tool=None
            # or tool="None". Both are valid termination signals — do not treat them
            # as invalid-tool errors (fixes the "WARNING: LLM chose invalid tool: None"
            # edge case shown in the live regression run slide E/E).
            if finish_flag:
                logging.info("[LLM Planner] finish=True — terminating workflow cleanly.")
                break
            ...
            # finish_flag already handled before tool dispatch — workflow exits cleanly.
            # Keep this secondary check for the case where finish=True accompanies a
            # valid final tool call (e.g., generate_recommendations with finish=True).
            if finish_flag:
                logging.info("[LLM Planner] Workflow complete after final tool execution.")
                break
    ...
    def _execute_preprocessing_step(self):
        ...
        if self.problem_type == 'anomaly_detection':
            ...
            # PATCH 2: Pass ID columns through as non-model identifiers so the
            # OptimizationAgent can map predictions back to machine names.
            ...
            if id_passthrough:
                logging.info(f"Including ID columns as pass-through: {id_passthrough}")
                analysis_data = self.raw_data[self.feature_columns + [self.target_column] + id_passthrough]
                protected_cols = self.feature_columns + id_passthrough
    ...
    def _ask_llm_for_anomaly_params(self, dataset_summary: str, use_decision_llm: bool = False) -> Dict[str, Any]:
        """
        Ask the decision SLM to suggest anomaly detection hyperparameters.
        
        THIS IS THE ONLY RETAINED SLM POSITION (SLM 3b) in the architecture.
        
        Why this SLM call is irreplaceable:
        - contamination depends on multi-signal interaction: outlier fraction + 
          feature distribution shape + FP vs. FN trade-off
        - Two datasets with identical outlier % may need very different values - 
          a scalar rule cannot generalise
        - SLM provides an auditable 'reason' field - operator can read and challenge
        - Output is bounded: contamination clipped to [0.001, 0.2]; n_estimators floored at 50
        - HITL gate immediately follows - operator approves before IsolationForest runs
        - Fallback: sklearn defaults (contamination='auto') if Ollama unavailable
        
        Expected JSON response: {"contamination": 0.05, "n_estimators": 200, "reason": "..."}
        
        Args:
            dataset_summary: Compact dataset profile for the prompt
            use_decision_llm: Whether to use decision SLM (True) or planner LLM (False)
        
        Returns:
            dict: {"contamination": float|'auto', "n_estimators": int, "reason": str}
        """
        prompt = (
